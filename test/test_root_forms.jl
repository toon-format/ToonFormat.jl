...
@testset "Root Form Detection Tests (Task 8)" begin
    @testset "Root Array Detection (Requirement 11.1)" begin
        ...
        result = TOON.decode("[2\t]: a\tb")
        @test result == ["a", "b"]
        
        # Root array with pipe delimiter
        result = TOON.decode("[2|]: x|y")
        @test result == ["x", "y"]
        
        # Root array with tabular format
        result = TOON.decode("[2]{name,age}:\n  Alice,30\n  Bob,25")
    ...
    @testset "Single Primitive Detection (Requirement 11.2)" begin
        ...
        result = TOON.decode("3.14")
        @test result == 3.14
        @test isa(result, Number)
        
        # Single boolean primitive - true
        result = TOON.decode("true")
        @test result == true
        @test isa(result, Bool)
        
        # Single boolean primitive - false
        result = TOON.decode("false")
        @test result == false
        @test isa(result, Bool)
        
        # Single null primitive
        result = TOON.decode("null")
        @test result === nothing
        
        # Single quoted string
        result = TOON.decode("\"hello world\"")
    ...
    @testset "Object Detection (Requirement 11.3)" begin
        ...
        # Object with array
        result = TOON.decode("items[2]: a,b")
    ...
    @testset "Invalid Multi-Primitive in Strict Mode" begin
        ...
        @test_throws Exception TOON.decode("hello\nworld")
        @test_throws Exception TOON.decode("42\n3.14")
        @test_throws Exception TOON.decode("true\nfalse")
        ...
        @test_nowarn TOON.decode("hello\nworld", options=TOON.DecodeOptions(strict=false))
    end
    ...
    @testset "Edge Cases and Ambiguous Inputs" begin
        ...
        result = TOON.decode("[1]: x")
        @test isa(result, Vector{Any})
        @test result == ["x"]
        
        # Multiple root arrays should be treated as object with array keys
        result = TOON.decode("[3]: 1,2,3\n[2]: a,b", options=TOON.DecodeOptions(strict=false))
