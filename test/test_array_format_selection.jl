...
@testset "Array Format Selection" begin
    @testset "Requirement 6.1: Primitive arrays use inline format" begin
        ...
        # Float array
        result = TokenOrientedObjectNotation.encode([1.5, 2.5, 3.5])
        @test occursin("[3]:", result)
        ...
        # Boolean array
        result = TokenOrientedObjectNotation.encode([true, false, true])
        @test occursin("[3]:", result)
        @test occursin("true,false,true", result)
        
        # Null array
        result = TokenOrientedObjectNotation.encode([nothing, nothing])
        @test occursin("[2]:", result)
        @test occursin("null,null", result)
        
        # Mixed primitive types
        result = TokenOrientedObjectNotation.encode([1, "hello", true, nothing, 3.14])
        @test occursin("[5]:", result)
        @test occursin("1,hello,true,null,3.14", result)
        
        # Single element primitive array
        result = TokenOrientedObjectNotation.encode([42])
