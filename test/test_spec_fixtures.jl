"""
Test TOON.jl against official TOON specification test fixtures
                ...
                for test in fixture.tests
                    @testset "$(test.name)" begin
                        ...
                        if get(test, :shouldError, false)
                            @test_throws Exception TOON.encode(input; options=options)
                        else
                            result = TOON.encode(input; options=options)
            ...
            @testset "$(fixture.description)" begin
                for test in fixture.tests
                    @testset "$(test.name)" begin
                        ...
                        if get(test, :shouldError, false)
                            @test_throws Exception TOON.decode(input; options=options)
                        else
                            result = TOON.decode(input; options=options)
                            @test result == expected
                        end
                    end
                end
