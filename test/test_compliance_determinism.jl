...
@testset "Compliance: Determinism Tests" begin
    ...
    @testset "Object Determinism" begin
        ...
        # Multiple encodings
        results = [ToonFormat.encode(obj) for _ in 1:5]
    ...
    @testset "Array Determinism" begin
        ...
        # Multiple encodings
        results = [ToonFormat.encode(arr) for _ in 1:5]
        @test all(r == results[1] for r in results)
    end
    ...
    @testset "Options Determinism" begin
        ...
        @test ToonFormat.encode(arr, options=opts) == ToonFormat.encode(arr, options=opts)
        ...
        @test ToonFormat.encode(arr, options=opts1) != ToonFormat.encode(arr, options=opts2)
    end
end
