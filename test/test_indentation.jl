...
@testset "Indentation and Whitespace" begin
    ...
    @testset "Requirement 9.7: Strict mode validates indentation as exact multiple" begin
        ...
        valid_input = "a:\n  b: 1"
        @test_nowarn ToonFormat.decode(valid_input, options=ToonFormat.DecodeOptions(indent=2, strict=true))
        
        # Valid indentation (multiples of 4)
        valid_input = "a:\n    b: 1"
        @test_nowarn ToonFormat.decode(valid_input, options=ToonFormat.DecodeOptions(indent=4, strict=true))
        
        # Invalid indentation (3 spaces with indent=2)
        invalid_input = "a:\n   b: 1"
        @test_throws Exception ToonFormat.decode(invalid_input, options=ToonFormat.DecodeOptions(indent=2, strict=true))
        ...
        invalid_input = "a:\n  b: 1"
        @test_throws Exception ToonFormat.decode(invalid_input, options=ToonFormat.DecodeOptions(indent=4, strict=true))
        
        # Non-strict mode allows non-multiples
        lenient_input = "a:\n   b: 1"
        result = ToonFormat.decode(lenient_input, options=ToonFormat.DecodeOptions(indent=2, strict=false))
        @test haskey(result, "a")
        
        # Deep nesting with valid indentation
        deep_valid = "a:\n  b:\n    c:\n      d: 1"
        @test_nowarn ToonFormat.decode(deep_valid, options=ToonFormat.DecodeOptions(indent=2, strict=true))
        
        # Deep nesting with invalid indentation at one level
        deep_invalid = "a:\n  b:\n   c:\n      d: 1"
        @test_throws Exception ToonFormat.decode(deep_invalid, options=ToonFormat.DecodeOptions(indent=2, strict=true))
    end
    
    @testset "Requirement 9.8: Strict mode rejects tabs in indentation" begin
        # Tab in indentation
        tab_input = "\tvalue: 1"
        @test_throws Exception ToonFormat.decode(tab_input, options=ToonFormat.DecodeOptions(strict=true))
        
        # Tab mixed with spaces
        mixed_input = "  \tvalue: 1"
        @test_throws Exception ToonFormat.decode(mixed_input, options=ToonFormat.DecodeOptions(strict=true))
        
        # Tab at deeper level
        nested_tab = "a:\n\tb: 1"
        @test_throws Exception ToonFormat.decode(nested_tab, options=ToonFormat.DecodeOptions(strict=true))
        ...
        @test_nowarn ToonFormat.decode(array_with_tab, options=ToonFormat.DecodeOptions(strict=true))
        
        # Tab in string value is OK (not indentation)
        string_with_tab = "text: \"hello\tworld\""
        @test_nowarn ToonFormat.decode(string_with_tab, options=ToonFormat.DecodeOptions(strict=true))
        
        # Non-strict mode is more lenient - tabs are allowed but treated as single space
        # This is implementation-dependent behavior
        result = ToonFormat.decode("\tvalue: 1", options=ToonFormat.DecodeOptions(strict=false))
    ...
    @testset "Edge Cases: Indentation and Whitespace" begin
        ...
        result = ToonFormat.decode(input_with_blanks, options=ToonFormat.DecodeOptions(strict=false))
        @test haskey(result, "a")
