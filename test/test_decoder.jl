...
@testset "Decoder Tests" begin
    @testset "API Tests" begin
        @test_nowarn TokenOrientedObjectNotation.decode("items[5]: a,b,c", options=TokenOrientedObjectNotation.DecodeOptions(strict=false))

        # Test decode with custom indent size
        input = "parent:\n    child:\n        value: 42"
        result = TokenOrientedObjectNotation.decode(input, options=TokenOrientedObjectNotation.DecodeOptions(indent=4))
        @test result["parent"]["child"]["value"] == 42

        # Test decode returns native Dict (OrderedDict to preserve key order)
        result = TokenOrientedObjectNotation.decode("id: 123")
    ...
    @testset "Error Handling" begin
        ...
        @test_throws Exception TokenOrientedObjectNotation.decode("items[5]: a,b,c")
        ...
        @test_throws ArgumentError TokenOrientedObjectNotation.decode("text: \"test\\a\"")
        @test_throws ArgumentError TokenOrientedObjectNotation.decode("text: \"test\\b\"")
        @test_throws ArgumentError TokenOrientedObjectNotation.decode("text: \"test\\f\"")
        @test_throws ArgumentError TokenOrientedObjectNotation.decode("text: \"test\\v\"")
        @test_throws ArgumentError TokenOrientedObjectNotation.decode("text: \"test\\0\"")
        
        # Test unterminated escape sequence
        @test_throws ArgumentError TokenOrientedObjectNotation.decode("text: \"test\\\"")
    end

    @testset "Spec Edge Cases" begin
        # Leading zero treated as string
        result = TokenOrientedObjectNotation.decode("code: 05")
        @test result["code"] == "05"
        @test isa(result["code"], String)

        # Leading zeros in array (root array)
        result = TokenOrientedObjectNotation.decode("[3]: 01,02,03")
        @test result == ["01", "02", "03"]
        @test all(isa(x, String) for x in result)

        # Single zero is number
        result = TokenOrientedObjectNotation.decode("value: 0")
        @test result["value"] == 0
        @test isa(result["value"], Number)

        # Zero point zero is number
        result = TokenOrientedObjectNotation.decode("value: 0.0")
        @test result["value"] == 0.0
        @test isa(result["value"], Number)

        # Exponent notation accepted
        @test TokenOrientedObjectNotation.decode("value: 1e-6")["value"] ≈ 1e-6
        @test TokenOrientedObjectNotation.decode("value: -1E+9")["value"] ≈ -1E+9
        @test TokenOrientedObjectNotation.decode("value: 2.5e3")["value"] ≈ 2500.0

        # Exponent notation in array (root array)
        result = TokenOrientedObjectNotation.decode("[3]: 1e2,2e-1,3E+4")
        @test result[1] ≈ 100.0
        @test result[2] ≈ 0.2
        @test result[3] ≈ 30000.0

        # Array order preserved (root array)
        result = TokenOrientedObjectNotation.decode("[5]: 5,1,9,2,7")
    ...
    @testset "Escape Sequence Handling (Requirements 3.1, 3.2)" begin
        ...
        @test TokenOrientedObjectNotation.decode("text: \"hello\\nworld\"")["text"] == "hello\nworld"
        @test TokenOrientedObjectNotation.decode("text: \"col1\\tcol2\"")["text"] == "col1\tcol2"
        @test TokenOrientedObjectNotation.decode("text: \"line1\\rline2\"")["text"] == "line1\rline2"
        @test TokenOrientedObjectNotation.decode("text: \"path\\\\to\\\\file\"")["text"] == "path\\to\\file"
        @test TokenOrientedObjectNotation.decode("text: \"say \\\"hello\\\"\"")["text"] == "say \"hello\""
        
        # Test multiple escape sequences
        @test TokenOrientedObjectNotation.decode("text: \"test\\n\\r\\t\\\\\\\"value\\\"\"")["text"] == "test\n\r\t\\\"value\""
        
        # Test escape sequences in arrays
        result = TokenOrientedObjectNotation.decode("[3]: \"a\\nb\",\"c\\td\",\"e\\\\f\"")
        @test result[1] == "a\nb"
        @test result[2] == "c\td"
        @test result[3] == "e\\f"
